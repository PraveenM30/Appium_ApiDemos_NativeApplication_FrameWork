...
public class AndroidActions extends AppiumUtils {
	...
	public void ScrollTillElement(WebElement ele) {
		while (!ele.isDisplayed()) {
			((JavascriptExecutor) driver).executeScript("mobile: scrollGesture", ImmutableMap.of("left", 100, "top",
					100, "width", 500, "height", 800, "direction", "down", "percent", 1.0));
		}
	}
	...
	public void dragAndDropEle(WebElement sourceEle,WebElement targetEle,long MilliSec) {
		Actions action = new Actions(driver);
		action.clickAndHold(sourceEle);
		action.pause(MilliSec);
